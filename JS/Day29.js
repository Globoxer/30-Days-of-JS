...
class UI{
    ...
    static validateInputs(){
        ...
        var licensePlateRegex = /^(?:[A-Z]{2}-\d{2}-\d{2})|(?:\d{2}-[A-Z]{2}-\d{2})|(?:\d{2}-\d{2}-[A-Z]{2}|(?:[A-Z]{2}-[A-Z]{2}-\d{2}))$/
        if(owner === '' || car === '' || licensePlate === '' || entryDate === '' || exitDate === ''){
            UI.showAlert('All fields must be filled!', 'danger')
            return false
        }
        if(exitDate < entryDate){
            UI.showAlert('Cars must enter before they can leave!', 'danger')
            return false
        }
        if(!licensePlateRegex.test(licensePlate)){
            UI.showAlert('License Plate must be like NN-NN-LL, NN-LL-NN, LL-NN-NN, LL-LL-NN', 'danger')
            return false
        }
        return true
    }
